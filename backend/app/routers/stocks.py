...
@router.get("/scan/near-ema60/watchlist")
async def near_ema60_watchlist():
    try:
        from app.services.signal_tracking import get_ema60_watchlist_view
        results = await run_in_threadpool(get_ema60_watchlist_view)
        return {"count": len(results), "stocks": results}
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
async def upcoming_dividends(days: int = Query(default=60, le=180), force: bool = Query(default=False)):
    try:
        rows = await run_in_threadpool(get_upcoming_dividends, days, force)
...
@router.get("/{ticker}/intraday-chart")
async def get_intraday_chart_endpoint(ticker: str):
    try:
