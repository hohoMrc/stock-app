...
from app.services import paper_daytrade as svc

router = APIRouter(prefix="/api/paper-daytrade", tags=["paper-daytrade"])
...
async def place_order(body: OrderBody, authorization: str | None = Header(None)):
    ...
    try:
        return await run_in_threadpool(svc.place_daytrade_order, user_id, body.ticker, body.side, body.lots, body.price)
    except svc.PaperDaytradeError as e:
